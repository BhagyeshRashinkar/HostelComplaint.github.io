...
app.post("/", function (req, res) {
  ...
  registerComplaint
    ...
    .then(res.status(201).redirect("/table"))
    .catch((err) => {
      res
        .status(500)
        .send({ message: err.message || "Data can't be sent to database" });
    });
});
...
app.post("/hosteltabl/:_id", function (req, res) {
  if (!req.body) {
    return res.status(400).send("Data to update status can't be found");
  } else {
    ...
    Register.updateOne(
      ...
      .then((data) => {
        if (!data) {
          res
            .status(404)
            .send(`Cannot update complaint status of user with ${id}`);
        } else {
